...
LiquidCrystal hd44780(rs, en, d4, d5, d6, d7);
...
uint32_t SPIIdleTimer;        //Tracks how long the SPI bus has been idle for1
...
uint8_t i2cCheckCount = 0;      //Tracks what check we're up to.
...
void setup() {
  ...
  analogWrite(backlightPin, 255); //0-255
  analogWrite(contrastPin, 120); //0-255
...
void loop() {
  ...
  if (QueueRxPos != QueueProcessPos) {
    switch (RxCommandQueue[QueueProcessPos]) {
      ...
      case XeniumSetCursorPosition: //Sets the row and column of cursor.
      ...
      case XeniumSetBacklight:
        ...
        if ((QueueRxPos-QueueProcessPos)>1 || (QueueRxPos-1)<QueueProcessPos) { //ensure the command is complete ISSUE HERE. What is brightness is zero?
      ...
      case XeniumCursorMove:
        //The following 2 bytes after the set the direction to move the cursor
  ...
  if (i2cBusy() == 0 &&
    ...
    //Read the CPU and GPU temps directly from the ADM1032 System Temperature Monitor then print to LCD
    if (readSMBus(ADM1032_ADDRESS, ADM1032_CPU, &rxBuffer[0], 1) == 0 &&
        readSMBus(ADM1032_ADDRESS, ADM1032_GPU, &rxBuffer[1], 1) == 0) {
      if (rxBuffer[0] < 200 && rxBuffer[1] < 200 && rxBuffer[0] > 0 && rxBuffer[1] > 0) {
        snprintf(lineBuffer, sizeof lineBuffer, "CPU:%3u%cC GPU:%3u%cC ", rxBuffer[0], (char)223, rxBuffer[1], char(223)); //char(223)=degree symbol
